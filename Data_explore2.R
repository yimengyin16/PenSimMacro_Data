...
bind_rows((df_stock_y$dl_gbond_o[-1]*100)   %>% describe(),
	        (df_stock_y$dl_cbond_o[-1]*100)   %>% describe(),
					(df_stock_y$return_tot_o[-1]*100) %>% describe()) %>% 
	mutate(varname = c('gbond_y', 'cbond_y', 'stock_y'))
...
cor(df_stock_y[-1, ] %>% select(dl_cbond_o, return_tot_o)) # corp bond: 0.28
cor(df_stock_y[-1, ] %>% select(dl_gbond_o, return_tot_o)) # gov bond:  0.21

cor(df_stock_y[23:63,]  %>% select(dl_cbond_o, return_tot_o)) # corp bond: 0.28
cor(df_stock_y[23:63, ] %>% select(dl_gbond_o, return_tot_o)) # gov bond:  0.21
