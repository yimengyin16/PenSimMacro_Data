...
# Real terms based on CPI-U (Urban's calculation BEA real GSP starts from 1987)
df_real <- 
	df_RevGSP %>%
...
# Real terms based on GDP price index
df_real2 <- 
	df_RevGDP_GDPCTPI %>%
	select(state_abb, year, 
...
#**********************************************************************
#     Descriptive analysis 1: decomposition of GDP and tax revenue ####
#**********************************************************************
df_temp <- 
df_real2 %>% 
	...
	mutate(GDP_FRED = GDP_FRED * 1e6,
		     ...
		     GDP_logtrend_hp = hpfilter(log(GDP_FRED), freq = 100)$trend,
				 ...
				 PIT_logtrend_hp = hpfilter(log(tax_indivIncome_real2_state), freq = 100)$trend,
				 PIT_logcycle_hp = hpfilter(log(tax_indivIncome_real2_state), freq = 100)$cycle,
				 PIT_cycle_fct   = exp(PIT_logcycle_hp),
				 
				 salesgen_log         = log(tax_sales_gen_real2_state),
				 salesgen_logtrend_hp = hpfilter(log(tax_sales_gen_real2_state), freq = 100)$trend,
				 salesgen_logcycle_hp = hpfilter(log(tax_sales_gen_real2_state), freq = 100)$cycle,
				 salesgen_cycle_fct   = exp(salesgen_logcycle_hp),
				 
				 salessel_log         = log(tax_sales_select_real2_state),
				 salessel_logtrend_hp = hpfilter(log(tax_sales_select_real2_state), freq = 100)$trend,
				 salessel_logcycle_hp = hpfilter(log(tax_sales_select_real2_state), freq = 100)$cycle,
				 salessel_cycle_fct   = exp(salessel_logcycle_hp),
				 
				 nonPITsalestot_log         = log(tax_nonPITsalestot_real2_state),
				 nonPITsalestot_logtrend_hp = hpfilter(log(tax_nonPITsalestot_real2_state), freq = 100)$trend,
				 nonPITsalestot_logcycle_hp = hpfilter(log(tax_nonPITsalestot_real2_state), freq = 100)$cycle,
				 nonPITsalestot_cycle_fct   = exp(nonPITsalestot_logcycle_hp),
				 
				 propertyLoc_log         = log(tax_property_real2_local),
				 propertyLoc_logtrend_hp = hpfilter(log(tax_property_real2_local), freq = 100)$trend,
				 propertyLoc_logcycle_hp = hpfilter(log(tax_property_real2_local), freq = 100)$cycle,
				 propertyLoc_cycle_fct   = exp(propertyLoc_logcycle_hp),
				 
				 PIT_GDP_trend = exp(PIT_logtrend_hp - GDP_logtrend_hp),
				 salesgen_GDP_trend = exp(salesgen_logtrend_hp - GDP_logtrend_hp),
				 salessel_GDP_trend = exp(salessel_logtrend_hp - GDP_logtrend_hp),
				 nonPITsalestot_GDP_trend = exp(nonPITsalestot_logtrend_hp - GDP_logtrend_hp),
				 propertyLoc_GDP_trend = exp(propertyLoc_logtrend_hp - GDP_logtrend_hp)
				 )


# level and trend
df_temp %>% # GDP
	select(year, GDP_log, GDP_logtrend_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point"))

df_temp %>% #PIT
	select(year, PIT_log, PIT_logtrend_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point"))

df_temp %>% #gen sales
	select(year, salesgen_log, salesgen_logtrend_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point"))
...
df_temp %>% 
	select(year, GDP_logcycle_hp, salesgen_logcycle_hp, salessel_logcycle_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point")) + 
	geom_hline(yintercept = 0, linetype = 2)

df_temp %>% 
	select(year, GDP_logcycle_hp, nonPITsalestot_logcycle_hp, propertyLoc_logcycle_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point")) + 
	geom_hline(yintercept = 0, linetype = 2)
...
# Trends
df_temp %>% # PIT vs gen sales
	select(year, PIT_logtrend_hp, salesgen_logtrend_hp) %>% 
	gather(var, value, -year) %>% 
	qplot(x = year, y = value, color = var, data = ., geom = c("line", "point")) 
	# geom_hline(yintercept = 1, linetype = 2)


df_temp %>% # PIT vs gen sales as % of GDP
		select(year, 
					 PIT_GDP_trend, 
					 salesgen_GDP_trend, 
					 salessel_GDP_trend,
...
df_real_reg %>% 
	select(state_abb, year, 
				 ...
				 LCapStock_TRI_real, 
				 LTGBond_TRI_real,   
				 SP500_return_nyu_real, 
				 TBond_return_nyu_real, 
				 capgains_chg_real, 
				 tax_gains_chg,
				 tax_indivIncome_real_state) %>% 
	filter(state_abb == "US")
...
fig_returnCapGains_real <- 
	df_temp %>% 
	select(state_abb, year, LCapStock_TRI_real, capgains_chg_real) %>%  
	gather(var, value, -state_abb, -year) %>% 
	mutate(var = factor(var, levels = c("LCapStock_TRI_real", "capgains_chg_real"),
...
df_temp %>% 
	select(state_abb, year, LTGBond_TRI_real, capgains_chg_real) %>%  
	gather(var, value, -state_abb, -year) %>% 
	qplot(x = year, y = value, color = var, data=.,  geom = c("line", "point")) + theme_bw() + 
	scale_x_continuous(breaks = seq(1950, 2020, 5))
...
fig_capgainsTax_real <- 
	df_temp	%>% 
	filter(year >= 1988) %>% 
	select(state_abb, year, capgains_chg_real, LCapStock_TRI_real, tax_indivIncome_real_state) %>% 
	mutate(capgains_chg_real = lag(capgains_chg_real),
				 LCapStock_TRI_real = lag(LCapStock_TRI_real)) %>% 
	gather(var, value, -state_abb, -year) %>% 
	mutate(var = factor(var, levels = c("capgains_chg_real", "LCapStock_TRI_real", "tax_indivIncome_real_state"),
...
mod_PIT_GSPd91 <- 
	df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_indivIncome_real_state ~ RGSP + RGSP:after91, data = .) 
mod_PIT_GSPd91 %>% summary
# 95, 91 dummies not significant either
...
mod_PIT_GSP.Lagstock_real <- 
	...
	#mutate(RGSP = RGSP_ext) %>% 
	mutate(RGSP = GDP_FRED) %>% 
	lm(tax_indivIncome_real_state ~ RGSP + LagLCapStock_TRI_real, data = .)
...
mod_PIT_GSPd99.Lagstockd99_real <- 
	...
	mutate(RGSP = GDP_FRED) %>% 
	#mutate(RGSP = RGSP_ext) %>% 
	lm(tax_indivIncome_real_state ~ RGSP + RGSP:after99 + LagLCapStock_TRI_real + LagLCapStock_TRI_real:after99, data =.)
mod_PIT_GSPd99.Lagstockd99_real %>% summary


mod_PIT_GSP.Lagstockd99 <- 
	df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>% 
	#mutate(RGSP = RGSP_ext) %>% 
	lm(tax_indivIncome_real_state ~ RGSP + LagLCapStock_TRI + LagLCapStock_TRI:after99, data =.) 
mod_PIT_GSP.Lagstockd99 %>% summary

mod_PIT_GSP.Lagstockd99_real <- 
	df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>% 
	#mutate(RGSP = RGSP_ext) %>% 
	lm(tax_indivIncome_real_state ~ RGSP + LagLCapStock_TRI_real + LagLCapStock_TRI_real:after99 + recession_recent2, data =.) 
...
mod_PIT_GSP.Lagcapgain_real <- 
	df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_indivIncome_real_state ~ RGSP + Lagcapgains_chg_real + d87_88, data = .) 
mod_PIT_GSP.Lagcapgain_real %>% summary


mod_PIT_GSPd99.Lagcapgaind99 <-
df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_indivIncome_real_state ~ RGSP + RGSP:after99 + Lagcapgains_chg +  Lagcapgains_chg:after99, data = .) 
mod_PIT_GSPd.Lagcapgaind99 %>% summary
# GSP before 1999 and SP500 after 1999 (10% level) are significant

mod_PIT_GSPd99.Lagcapgaind99_real <-
	df_reg_temp %>% filter(state_abb == "US") %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_indivIncome_real_state ~ RGSP + RGSP:after99 + Lagcapgains_chg_real+  Lagcapgains_chg_real:after99, data = .) 
...
mod_PIT_GSP.Lagcapgaind99_real <- 
	df_reg_temp %>% filter(state_abb == "US") %>% 
	lm(tax_indivIncome_real_state ~ RGSP + Lagcapgains_chg_real + Lagcapgains_chg_real:after99, data = .) 
...
Table_PIT_param_nomStock <- 
bind_rows(
   mod_PIT_GSP               %>% tidy() %>% mutate(mod = "PIT_GSP"),
   mod_PIT_GSPd99              %>% tidy() %>% mutate(mod = "PIT_GSPd99"),
   mod_PIT_GSP.Lagstock      %>% tidy() %>% mutate(mod = "PIT_GSP.Lagstock"),
   mod_PIT_GSPd99.Lagstockd99    %>% tidy() %>% mutate(mod = "PIT_GSPd99.Lagstockd99"),
   mod_PIT_GSP.Lagstockd99     %>% tidy() %>% mutate(mod = "PIT_GSP.Lagstockd99"),
   
   mod_PIT_GSP.Lagcapgain    %>% tidy() %>% mutate(mod = "PIT_GSP.Lagcapgain"),
   mod_PIT_GSPd99.Lagcapgaind99  %>% tidy() %>% mutate(mod = "PIT_GSPd99.Lagcapgaind99"),
   mod_PIT_GSP.Lagcapgaind99    %>% tidy() %>% mutate(mod = "PIT_GSP.Lagcapgaind99")
)

Table_PIT_glance_nomStock <- 
bind_rows(
   mod_PIT_GSP               %>% glance() %>% mutate(mod = "PIT_GSP"),
   mod_PIT_GSPd99              %>% glance() %>% mutate(mod = "PIT_GSPd99"),
   mod_PIT_GSP.Lagstock      %>% glance() %>% mutate(mod = "PIT_GSP.Lagstock"),
   mod_PIT_GSPd99.Lagstockd99    %>% glance() %>% mutate(mod = "PIT_GSPd99.Lagstockd99"),
   mod_PIT_GSP.Lagstockd99    %>% glance() %>% mutate(mod = "PIT_GSP.Lagstockd99"),
   
   mod_PIT_GSP.Lagcapgain    %>% glance() %>% mutate(mod = "PIT_GSP.Lagcapgain"),
   mod_PIT_GSPd99.Lagcapgaind99  %>% glance() %>% mutate(mod = "PIT_GSPd99.Lagcapgaind99"),
   mod_PIT_GSP.Lagcapgaind99   %>% glance() %>% mutate(mod = "PIT_GSP.Lagcapgaind99")
)
...
Table_PIT_param_realStock <- 
	bind_rows(
		...
		mod_PIT_GSPd99.Lagstockd99_real %>% tidy() %>% mutate(mod = "PIT_GSPd99.Lagstockd99"),
		mod_PIT_GSP.Lagstockd99_real    %>% tidy() %>% mutate(mod = "PIT_GSP.Lagstockd99"),
		
		mod_PIT_GSP.Lagcapgain_real       %>% tidy() %>% mutate(mod = "PIT_GSP.Lagcapgain"),
		mod_PIT_GSPd99.Lagcapgaind99_real %>% tidy() %>% mutate(mod = "PIT_GSPd99.Lagcapgaind99"),
		mod_PIT_GSP.Lagcapgaind99_real    %>% tidy() %>% mutate(mod = "PIT_GSP.Lagcapgaind99")
	)
...
Table_PIT_glance_realStock <- 
	bind_rows(
		...
		mod_PIT_GSPd99.Lagstockd99_real %>% glance() %>% mutate(mod = "PIT_GSPd99.Lagstockd99"),
		mod_PIT_GSP.Lagstockd99_real    %>% glance() %>% mutate(mod = "PIT_GSP.Lagstockd99"),
		
		mod_PIT_GSP.Lagcapgain_real        %>% glance() %>% mutate(mod = "PIT_GSP.Lagcapgain"),
		mod_PIT_GSPd99.Lagcapgaind99_real  %>% glance() %>% mutate(mod = "PIT_GSPd99.Lagcapgaind99"),
		mod_PIT_GSP.Lagcapgaind99_real     %>% glance() %>% mutate(mod = "PIT_GSP.Lagcapgaind99")
	)


write.xlsx2(Table_PIT_param_nomStock, file = paste0("policyBrief_out/", "Table_PIT_param_nomStock.xlsx"), sheet = "PIT_param" )
write.xlsx2(Table_PIT_glance_nomStock, file = paste0("policyBrief_out/", "Table_PIT_param_nomStock.xlsx"), sheet = "PIT_glance", append = TRUE)

write.xlsx2(Table_PIT_param_realStock, file = paste0("policyBrief_out/", "Table_PIT_param_realStock.xlsx"), sheet = "PIT_param" )
write.xlsx2(Table_PIT_glance_realStock, file = paste0("policyBrief_out/", "Table_PIT_param_realStock.xlsx"), sheet = "PIT_glance", append = TRUE)
...
fig_salesGDP <- 
df_reg_temp %>% 
	select(state_abb, year, GDP_FRED, tax_sales_gen_real_state, tax_sales_select_real_state) %>% 
	filter(year >= 1977, state_abb %in% "US") %>% 
	gather(var, value, -state_abb, -year) %>% 
	mutate(var = factor(var, levels = c("GDP_FRED", "tax_sales_gen_real_state", "tax_sales_select_real_state"),
											labels = c("GDP growth", "Real growth of general sales tax", "Real growth of select sales tax"))) %>%
	ungroup() %>% 
	#mutate(state_abb = factor(state_abb, levels = states_sales)) %>% 
	ggplot(aes(x = year, y = 100 *  value, color = var ))+ theme_bw() + RIG.themeLite() + # facet_grid(state_abb~.) + 
	geom_line() +
	geom_point() + 
	geom_hline(yintercept = 0, linetype = 2) + 
	scale_color_manual(values = c("darkgrey", "blue", "lightblue"))+
...
ggsave(paste0("policyBrief_out/", "fig_salesGDP.png"), fig_salesGDP, width = 10*0.8, height = 5*0.8)
...
#*******************************************************************************
#     Regression analysis US: Sales, long-term and cyclical                 ####
#*******************************************************************************
...
# GDP only
mod_salesgen_GSP <- 
	df_reg_temp %>% filter(state_abb == "US", year >=1985) %>% 
	#mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED, data = .) 
mod_salesgen_GSP %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_recent2*GDP_FRED, data =.) 
mod_salesgen_GSPrec %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1977) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_recent1, data = .) 
mod_salesgen_GSPrec %>% summary

mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1977) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_recent2, data = .) 
mod_salesgen_GSPrec %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1995) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_recent1, data = .) 
mod_salesgen_GSPrec %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1977) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + GDP_FRED:before1995 +  recession, data = .) 
mod_salesgen_GSPrec %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_all, data = .) 
mod_salesgen_GSPrec %>% summary
...
mod_salesgen_GSPrec <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_0810 + recession_0102, data = .) 
mod_salesgen_GSPrec %>% summary
...
# 	df_reg_temp %>% filter(state_abb == "US") %>% # with break dummy
# 	mutate(RGSP = GDP_FRED) %>%                                         
# 	lm(tax_sales_gen_real_state ~ RGSP + RGSP:after99 + RGSP*recession, data = .) 
# mod_salesgen_GSPd %>% summary




## general sales tax for the first draft of Policy brief
...
mod_salesgen_GSPrecAll85 <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_gen_real_state ~ GDP_FRED + recession_all2, data = .) 
...
write.xlsx2(Table_salesgen_new_param,  file = paste0("policyBrief_out/", "Table_salesgen_new_param.xlsx"), sheet = "sales_param" )
write.xlsx2(Table_salesgen_new_glance, file = paste0("policyBrief_out/", "Table_salesgen_new_param.xlsx"), sheet = "sales_glance", append = TRUE)





## select sales tax for the first draft of Policy brief

# GDP only
mod_salesselect_GSP <- 
	df_reg_temp %>% filter(state_abb == "US", year >=1985) %>% 
	#mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_select_real_state ~ GDP_FRED, data = .) 
mod_salesselect_GSP %>% summary
...
mod_salesselect_GSPrec85 <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_select_real_state ~ GDP_FRED + recession_recent2, data = .) 
...
mod_salesselect_GSPrecAll85 <- # with recession dummy
	df_reg_temp %>% filter(state_abb == "US", year >= 1985) %>% 
	mutate(RGSP = GDP_FRED) %>%
	lm(tax_sales_select_real_state ~ GDP_FRED + recession_all2, data = .) 
...
write.xlsx2(Table_salesselect_new_param,  file = paste0("policyBrief_out/", "Table_salesselect_new_param.xlsx"), sheet = "sales_param" )
write.xlsx2(Table_salesselect_new_glance, file = paste0("policyBrief_out/", "Table_salesselect_new_param.xlsx"), sheet = "sales_glance", append = TRUE)
