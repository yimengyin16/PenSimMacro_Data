...
load("policyBrief_out/df_sim_forward.RData")
...
df_risk <- 
	...
	mutate(
		...
		# ERC_tax_PITState_a1_high2   = cumany(ERC_tax_PITState_a1 >= 0.1),
		# ERC_tax_PITState_a1_hike   = cumany(na2zero(ERC_tax_PITState_a1 - lag(ERC_tax_PITState_a1, 2) >= 0.03)),
		# ERC_tax_PITState_a1_high15 = cumany(ERC_tax_PITState_a1 >= 0.15 ),
		# 
		# ERC_tax_salesState_a1_high2 = cumany(ERC_tax_salesState_a1>= 0.1),
		# ERC_tax_salesState_a1_hike = cumany(na2zero(ERC_tax_salesState_a1 - lag(ERC_tax_salesState_a1, 2) >= 0.03)),
		# ERC_tax_salesState_a1_high15 = cumany(ERC_tax_salesState_a1>= 0.15),
		# 
		# ERC_tax_localState_a1_high15 = cumany(ERC_tax_local_a1 >= 0.1),
		...
		ERC_tax_PITState_a1_high2   = cumany(ERC_tax_PITState_a1 >= 2 * ERC_tax_PITState_a1[year == 1] ),
		ERC_tax_PITState_a1_hike   = cumany(na2zero(ERC_tax_PITState_a1 - lag(ERC_tax_PITState_a1, 2) >= 0.03)),
		ERC_tax_PITState_a1_high15 = cumany(ERC_tax_PITState_a1 >= 1.5 * ERC_tax_PITState_a1[year == 1] ),

		ERC_tax_salesState_a1_high2 = cumany(ERC_tax_salesState_a1>= 2 * ERC_tax_salesState_a1[year == 1]),
		ERC_tax_salesState_a1_hike = cumany(na2zero(ERC_tax_salesState_a1 - lag(ERC_tax_salesState_a1, 2) >= 0.03)),
		ERC_tax_salesState_a1_high15 = cumany(ERC_tax_salesState_a1>= 1.5 * ERC_tax_salesState_a1[year == 1]),

		ERC_tax_localState_a1_high15 = cumany(ERC_tax_local_a1 >= 1.5 * ERC_tax_local_a1[year == 1]),
		ERC_tax_localState_a1_high2 = cumany(ERC_tax_local_a1 >= 2 * ERC_tax_local_a1[year == 1]),
		ERC_tax_localState_a1_hike = cumany(na2zero(ERC_tax_local_a1 - lag(ERC_tax_local_a1, 2) >= 0.03))
	...
	summarise(
		...
		ERC_tax_PITState_a1_high15 =  sum(ERC_tax_PITState_a1_high15, na.rm = T)/n(),
		...
		ERC_tax_salesState_a1_high15 =  sum(ERC_tax_salesState_a1_high15, na.rm = T)/n(),
		...
		ERC_tax_local_a1_high15 =  sum(ERC_tax_localState_a1_high15, na.rm = T)/n(),
...
df_risk %>% 
	filter(year == 30) %>% 
	select(runname, year, contains("_high15") )
...
## Fig: High ERC as a % of total tax revenue: 2x year 1 level
...
##  Fig: High ERC as a % of total tax revenue: 1.5x year 1 level

df_fig1 <- 
	df_risk %>% filter(runname %in% c("A_O30pA5_port70_30", "A_C15d_port70_30",
																		"B_O30pA5_port70_30", "B_C15d_port70_30")) %>% 
	select(year, DiscRate, Amort,  year, ERC_tax_PITState_a1_high15, ERC_tax_salesState_a1_high15, ERC_tax_local_a1_high15) 

df_fig2 <- 
	df_risk %>% filter(runname %in% c("A_O30pA5_normal", "A_C15d_normal",
																		"B_O30pA5_normal", "B_C15d_normal")) %>% 
	select(year, DiscRate, Amort,  year, ERC_tax_local_a1_high15) %>% 
	rename(ERC_tax_local_a1_high15_normal = ERC_tax_local_a1_high15)
df_fig2

df_fig <- left_join(df_fig1, df_fig2)
df_fig

fig.title <- "Probability of employer contribution as a percentage of total tax revenue \nrising above 1.5 times as high as the year-1 value at any time up to a given year"
fig.subtitle <- NULL #"Employer contribution is 5% of total tax revenue in year 1" 
fig_ERC_tax_high15 <- 
	df_fig %>% 
	select(year, DiscRate, Amort,  year, ERC_tax_PITState_a1_high15, ERC_tax_salesState_a1_high15, 
				 ERC_tax_local_a1_high15, ERC_tax_local_a1_high15_normal) %>% 
	gather(type, value, -DiscRate, -Amort, -year) %>% 
	mutate(type = factor(type, levels = c("ERC_tax_PITState_a1_high15", "ERC_tax_salesState_a1_high15", "ERC_tax_local_a1_high15", "ERC_tax_local_a1_high15_normal"),
											 labels = c("Income tax dominant state \nsimulated returns", "sales tax dominant state \nsimulated returns", 
											 					 "constant tax revenue growth \nsimulated returns", "constant tax revenue growth \nnormal returns"))) %>% 
	ggplot(aes(x = year, y = 100 * value,
						 color = type,
...
ggsave("policyBrief_out/fig_ERC_tax_high15.png", fig_ERC_tax_high15, width = 8, height = 9 )
ggsave("policyBrief_out/fig_ERC_tax_high2.png",  fig_ERC_tax_high2,  width = 8, height = 9 )
ggsave("policyBrief_out/fig_ERC_tax_hike.png",   fig_ERC_tax_hike,   width = 8, height = 9 )
...
Table_ERC_tax_high15 <- 
	df_risk %>% 
	filter(year == 30) %>% 
	select(ReturnDist, DiscRate, Amort, year, contains("_high15") ) %>% 
	arrange(ReturnDist, DiscRate, Amort)
...
write.xlsx2(Table_ERC_tax_high2, file = "policyBrief_out/Table_ERC_tax_risk.xlsx", sheetName = "ERC_tax_high2")
write.xlsx2(Table_ERC_tax_high15, file = "policyBrief_out/Table_ERC_tax_risk.xlsx", sheetName = "ERC_tax_high15", append = TRUE)
write.xlsx2(Table_ERC_tax_hike, file = "policyBrief_out/Table_ERC_tax_risk.xlsx", sheetName = "ERC_tax_hike", append = TRUE)
...
write.xlsx2(Table_risk_DC60, file = paste0(dir_out, "Table_penRisk.xlsx"), sheetName = "DC60")
write.xlsx2(Table_risk_DC75, file = paste0(dir_out, "Table_penRisk.xlsx"), sheetName = "DC75", append = TRUE)

ggsave(fig_FRdist, file = paste0(dir_out,  "fig_penRisk_FRdist.png"), width = 8, height = 8 )
ggsave(fig_ERCdist, file = paste0(dir_out, "fig_penRisk_ERCdist.png"), width = 8, height = 8 )

ggsave(fig_FR40less, file = paste0(dir_out, "fig_penRisk_FR40less.png"), width = 8, height = 5 )
ggsave(fig_ERChike,  file = paste0(dir_out, "fig_penRisk_ERChike.png"), width = 8, height = 5 )
