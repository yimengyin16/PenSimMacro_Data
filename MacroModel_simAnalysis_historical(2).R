...
load(paste0(dir_data_out, "MacroModel_sim_results_historical_real.RData"))

sim_results <- MacroModel_sim_results_historical_real
...
fn <- function(df, year_range, rolling_width, freq){
	#df_stock_m <- 
		...
		mutate(return_tot      = (1 + get_logReturn(LCapStock_TRI))^freq - 1,
					 ...
					 dl_cbond_real = (1 + get_logReturn(CBond_TRI_real))^freq - 1,
					 ...
					 dl_cbond_real_o = get_logReturn(CBond_TRI_real),
...
v_hist <- c(
mean = df_stock_y$return_tot_real_o %>% mean(na.rm = T),
sd   = df_stock_y$return_tot_real_o %>% sd(na.rm = T),
skew = df_stock_y$return_tot_real_o %>% skewness(na.rm = T),
kurto= df_stock_y$return_tot_real_o %>% kurtosis(na.rm = T, method = "sample_excess")
)
v_hist
...
df_stock_y$return_tot_o %>% kurtosis(na.rm = T, method = 'sample')
df_stock_y$return_tot_real_o %>% kurtosis(na.rm = T, method = 'sample')
...
v_hist <- c(
mean = df_stock_y$dl_cbond_real %>% mean(na.rm = T),
sd   = df_stock_y$dl_cbond_real %>% sd(na.rm = T),
skew = df_stock_y$dl_cbond_real %>% skewness(na.rm = T),
kurto= df_stock_y$dl_cbond_real %>% kurtosis(na.rm = T)
)
...
bind_rows(
	...
	df_stock_y %>% select(value = return_tot_real_o ) %>% mutate(type = "Historical", var = 'Stock return'),
	...
	df_stock_y           %>% select(value = dl_cbond_real_o) %>% mutate(type = "Historical", var = 'Bond return'),
