...
df_sim <- 
	...
	mutate(stockreturn_nom = stockreturn_real + infl,
				 bondreturn_nom  = bondreturn_real  + infl,
...
sim_geoMeans <- 
	...
	summarise(gdp_chg_geoMean          = get_geoReturn(gdp_chg),
						stockReturn_real_geoMean = get_geoReturn(stockreturn_nom),
						bondReturn_real_geoMean  = get_geoReturn(bondreturn_nom))
...
penSimInputs_returns <- 
  df_sim %>% 
	  select(sim, year, stockreturn_nom, bondreturn_nom) %>% 
	  mutate(return70_30_nom = stockreturn_nom * stock_pct + bondreturn_nom * bond_pct)
